...
class IntCodeStrategy(metaclass=abc.ABCMeta):
	"""An abstract class used for providing strategies for defining how the IntCodeComputer performs
...
class IntCodeAddStrategy(IntCodeStrategy):
	...
	def calculate(self, first_value, second_value):
		try:
			...
			print("A non-integer was given to the addition operation.")
...
class IntCodeMultiplyStrategy(IntCodeStrategy):
	...
	def calculate(self, first_value, second_value):
		try:
			...
			print("A non-integer was given to the multiplication operation.")

class IntCodeComputer:
	""" A tool for processing int-code instructions. Where int-code instructions use the following
	    format:

	        - A list of integers to be processed in sets of 4 integers, where in each set:
	            - Position 1 is a mathematical operation code or a halt code
	            - Position 2 is the position in the int-code instructions of the first value to be 
	              operated upon
	            - Position 3 is the position in the int-code instructions of the second value to be 
	              operated upon
	            - Position 4 is the position in the int-code instructions where the result of the 
		...
		Args:
			int_code_list (list): A list of integers representing a set of int-code instructions.
	"""
	...
	def compute_int_code(self):
		"""Process the int-code instructions provided to the IntCodeComputer instance, returns the value of position 0 of the int-code instructions at the end of the computation."""
		...
		try:
			...
			cursor_pos = 0			
			list_len = len(self._int_code_list)
			instr_sets_count = floor(list_len / self._INSTR_SET_LEN)				

			for i in range(0, instr_sets_count):
