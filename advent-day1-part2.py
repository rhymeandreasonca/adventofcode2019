...
class FuelCalculator:
    """A simple tool for calculating the total amount of fuel required to 
       launch a group of modules based on their individual masses and the 
       recursive mass of the fuel required by the initial launch.
    
    Args:
    module_masses (list): A list of integers representing the masses of 
    modules that need to be launched and for which fuel requirements need to be calculated.
    """
    ...
    def get_fuel_for_module(self, mass):
        """ Calculates the amount of fuel required to launch a single module based on its mass
         including the recursive mass of the fuel required to launch the fuel required to launch
         the module. """

        total_fuel = 0
         
        try:
            #int() automatically rounds down on cast
            total_fuel = int(mass/3) - 2

            if total_fuel > 0:
                fuel_for_fuel = self.get_fuel_for_module(total_fuel)
                total_fuel += fuel_for_fuel if fuel_for_fuel > 0 else 0
    ...
    def get_total_fuel(self):
        ...
        try:
            total_fuel = sum([self.get_fuel_for_module(module) for module in self._module_masses])
